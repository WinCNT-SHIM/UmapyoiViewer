...
UmaMusumeHairEffect::UmaMusumeHairEffect(std::shared_ptr<DXDevice> device, string m_FilePath)
	: EffectBase(device, m_FilePath, _T("UmaMusumeHair"))
{
	// 이펙트에서 PosColorTech라는 테크닉에 대한 포인터를 획득
	m_Tech = m_FX->GetTechniqueByName("UmaMusumeHairTech");
...
UmaMusumeHairEffect::~UmaMusumeHairEffect()
{
}

void UmaMusumeHairEffect::Release()
{
	m_World.Reset();
	m_View.Reset();
	m_Proj.Reset();
...
void UmaMusumeHairEffect::SetPerObjectData(std::shared_ptr<RenderingData>& renderingData)
{
...
void UmaMusumeHairEffect::SetTextureMap(TextureInfo& textureInfo)
{
	// diff
...
void UmaMusumeHairEffect::EffectRendering(std::shared_ptr<RenderingData>& renderingData)
{
	// 인풋 레이아웃 설정
...
void UmaMusumeHairEffect::SetMaterial(std::shared_ptr<MaterialInfo> materialInfo)
{
...
void UmaMusumeHairEffect::SetBoneTransforms(std::vector<XMMATRIX>& finalBoneTMList)
{
	// 예외 처리
	if (!m_BoneTMs)
		return;
