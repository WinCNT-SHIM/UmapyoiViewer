...
YumeWoKakeruScene::YumeWoKakeruScene()
	: SceneBase("YumeWoKakeruScene")
{
...
void YumeWoKakeruScene::BuildScene()
{
	// 메인 카메라 셋팅
	...
	// 리소스 로드
	// UmaMusume Stage
	ResourceManager::GetInstance()->LoadResource_ASEFile("Resource/ASEFile/pfb_env_live10113.ase");
	...
	// UmaMusume Assemble
	// Stage
	m_Stage = CreateGameObjectFromPrefab("Resource/ASEFile/pfb_env_live10113.ase");
	//m_Stage->GetComponent<Transform>()->SetScale(XMFLOAT3{ 0.0254f, 0.0254f, 0.0254f });
	m_Stage->GetComponent<Transform>()->SetScale(XMFLOAT3{ 0.02f, 0.02f, 0.02f });
	m_Stage->GetComponent<Transform>()->SetPosition(XMFLOAT3{ 0.0f, 0.0f, 2.00f });
...
void YumeWoKakeruScene::InputCharID()
{
	std::cout << std::endl;
...
bool YumeWoKakeruScene::CheckCharID(std::string charID)
{
	if (charID == "1002"
		|| charID == "1003"
		|| charID == "1004"
...
void YumeWoKakeruScene::Init()
{
	//std::string inputCharaID;
	//std::cout << "꿈을 걸다!";
	//std::cin >> inputCharaID;
	InputCharID();
...
void YumeWoKakeruScene::Release()
{
	SceneBase::Release();
}

void YumeWoKakeruScene::LateUpdate(float deltaTime)
{
