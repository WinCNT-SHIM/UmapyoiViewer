...
namespace ASEParser
{
	...
	public:
		...
		// 빛도 NodeTM이 두 개 있고, 카메라와 같은 이유로 이 변수가 있다.
		// ※(아무래도 좋지 않은 구조다. 바꾸자.)
		...
		vector<Bone*> m_BoneList;		// 이건 그냥 번호와 이름 관계이다. 이 안에 포인터를 넣어놓아도 괜찮겠구나(참조하기 편하게 - 그렇지 않으면 매번 이름으로 검색을 해야될지도. 아니면 인덱스가 있으니 상수 시간대로 접근을 할수도?)
		vector<VertexWeight*>	m_WVertexs;		// 버텍스 하나에 달라붙어있는 가중치들

		
		// ==================== Collision Detection 추가 코드 ====================
		// 메시의 최소/최대 버텍스를 저장하기 위한 변수
		XMFLOAT3 m_MinVertexOneMesh = { FLT_MAX, FLT_MAX, FLT_MAX };
		XMFLOAT3 m_MaxVertexOneMesh = { FLT_MIN, FLT_MIN, FLT_MIN };
		// 전체 오브젝트 하나에 대한 최대/최소 버텍스(공통된 값)
