...
public:
	...
	vector<ASEMaterial*> m_MaterialList;	// Material들의 리스트
	...
	// 메쉬로 옮기자
	///// 애니메이션
	Animation* m_TempAnimation;				// 한 오브젝트의 애니메이션 데이터(파싱용)
	//vector<Animation*> m_AnimationList;		// 그 데이터의 리스트


	// ==================== Collision Detection 추가 코드 ====================
	// 전체 오브젝트 하나에 대한 최대/최소 버텍스
...
private:
	...
	void Create_materialdata_to_list();	// 메트리얼 하나를 동적으로 생성하고, 그 포인터를 리스트에 넣는다.
	void Create_animationdata_to_list();// 애니메이션 데이터(한 개의 오브젝트 대응) 하나를 동적으로 생성하고, 리스트에 넣는다.
	void Create_onevertex_to_list();	// 버텍스 하나를 동적으로 생성하고, 그 포인터를 리스트에 넣는다.
};
